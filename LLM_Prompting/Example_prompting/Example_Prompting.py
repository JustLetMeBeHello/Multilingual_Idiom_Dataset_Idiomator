...
REQUEST_DIR = f"idioms_structured/Idiom_meanings/Example_Sentences/{language}/Batch_Requests"
RESULT_DIR = f"idioms_structured/Idiom_meanings/Example_Sentences/{language}/Batch_Results"
...
def submit_all(batch_paths):
    """Submit all batches and return list of (i, batch_id)"""
    submitted = []
    for i, path in batch_paths:
        ...
        time.sleep(0.5)  # avoid rate limiting on submission
    return submitted

# ---------------- POLL ALL ----------------

def poll_all(submitted):
    """Poll all in-flight batches until all are done."""
    pending = dict(submitted)  # i → batch_id
    ...
    while pending:
        ...
        for i, batch_id in pending.items():
            ...
            if status.status == "completed":
                ...
                still_pending[i] = batch_id  # still waiting
...
def merge_results():
    ...
    # Sort by last 4 digits of idiom_id, then by sense_number
    def sort_key(item):
